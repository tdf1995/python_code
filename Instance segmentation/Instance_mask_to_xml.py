...
root_path = r'E:\玉米\数据\train_set\Annotations'
                ...
                for filename in filenames:
                    if filename[-4:] =='.png':
                        ...
                        for i in range(len(contours)):
                            ...
                            _, mask_new = cv2.threshold(mask, i+1, 255, cv2.THRESH_BINARY)
                            mask_singe = cv2.subtract(mask_old, mask_new)
                            mask_old = mask_new
                            # cv2.imshow('1',mask_singe)
                            # cv2.waitKey(0)
                            contours, hierarchy = cv2.findContours(mask_singe,
                                                                   cv2.RETR_EXTERNAL,
                                                                   cv2.CHAIN_APPROX_NONE)
